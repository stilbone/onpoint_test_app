...
export const AboutScreen = props => { 
    ...
    let classNameTozoids = `${s.tozoids}`
    
    if( props.offset == -100 ){        
        classNameTozoids = `${s.tozoids} ${s.tozoidsAnimated}`    
    }   
    ...
    return( 
        <div className={s.main}>             
            <h2 className={s.title}>Текст сообщения</h2>
                <i className={s.customScrollLine}></i>
                <i className={s.customBackground}></i>
                ...
                <img src={Tozoids} className={ classNameTozoids } alt="tozoids image" /> 
        </div>
    )
}